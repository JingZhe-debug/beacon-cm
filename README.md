...
用户通过 Telegram Bot 验证后，系统自动记录 TG 信息至用户档案：

| 字段 | 说明 |
|:---|:---|
| `tgUserId` | Telegram 用户数字 ID |
| `tgUsername` | Telegram 用户名（含 @ 前缀） |

管理面板用户列表新增 **TG账号** 列，用户详情页展示 **TG 用户 ID** 和 **TG 用户名**。未绑定 TG 的用户统一显示"未绑定"。

搜索支持按 TG 用户 ID 或 TG 用户名查找用户。
